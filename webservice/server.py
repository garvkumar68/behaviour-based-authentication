...
TYPING_DATA_PATH = 'keystrokeDynamics/webservice/biometria.csv' 
...
def biometria():
    if request.method == 'POST':
        ...
        if len(data) > 18:
            data = data[:18]  # Limiting data to a maximum of 18 entries
        ...
        try:
            ...
            return jsonify({'biometric_cod': 'Could not register biometric data'})


@app.route('/treinar/biometria', methods=['POST'])
def treinar_biometria():
    if request.method == 'POST':
        response = dict(request.get_json())
        username = response['username']
        data = response['data']
        ...
        if user_id is None:  # If the user is not registered, set user_id to a default value
            user_id = 999
...
def auth2():
    ...
    resultado = classifica.knn_manhattan_sem_treino(amostra_digitacao)  # Passing amostra_digitacao argument
    ...
    return jsonify({'user_id': str(user_id), 'predict': resultado[0], 'accuracy': cross_val_score,
                    'result': str(match), 'algoritimo': resultado[2]})
